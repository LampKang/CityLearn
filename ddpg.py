...
class Agent:
    def __init__(self, env, building_info, observation_spaces = None, action_spaces = None):
        ...
        self.buffer = {i: ReplayBuffer() for i in range(self.n_buildings)}
        ...
        for i, (o, a) in enumerate(zip(observation_spaces, action_spaces)):
            ...
            self.actor[i] = Actor(self.state_dim[i], self.action_dim[i], self.action_bound[i])
            self.critic[i] = Critic(self.state_dim[i], self.action_dim[i])
            
            self.target_actor[i] = Actor(self.state_dim[i], self.action_dim[i], self.action_bound[i])
            self.target_critic[i]= Critic(self.state_dim[i], self.action_dim[i])

            actor_weights = self.actor[i].model.get_weights()
            critic_weights = self.critic[i].model.get_weights()
            self.target_actor[i].model.set_weights(actor_weights)
            self.target_critic[i].model.set_weights(critic_weights)
        
    
    def target_update(self, index):
        
        actor_weights = self.actor[index].model.get_weights()
        t_actor_weights = self.target_actor[index].model.get_weights()
        critic_weights = self.critic[index].model.get_weights()
        t_critic_weights = self.target_critic[index].model.get_weights()
        ...
        print(t_actor_weights)

        self.target_actor[index].model.set_weights(t_actor_weights)
        self.target_critic[index].model.set_weights(t_critic_weights)
    ...
    def replay(self):
        for i in range(self.n_buildings):
            states, actions, rewards, next_states, dones = self.buffer[i].sample()
            #print(action)
            target_q_values = self.target_critic[i].predict([next_states, self.target_actor[i].predict(next_states)])
            td_targets = self.td_target(rewards, target_q_values, dones)

            self.critic[i].train(states, actions, td_targets)
            
            s_actions = self.actor[i].predict(states)
            s_grads = self.critic[i].q_grads(states, s_actions)
            grads = np.array(s_grads).reshape((-1, self.action_dim[i]))
            self.actor[i].train(states, grads)
            self.target_update(i)
    ...
    def select_action(self, states):
        ...
        for i, state in enumerate(states):
            bg_noise = np.zeros(self.action_dim[i])
            action = self.actor[i].get_action(state)
            noise = self.ou_noise(bg_noise, dim=self.action_dim[i])
            for j in range(self.action_dim[i]):
                action[j] = action[j] + noise[j]
    ...
    def add_to_buffer(self, states, actions, rewards, next_states, dones):
        ...
        for i, (s, a, r, s_next, done) in enumerate(zip(states, actions, rewards, next_states, dones)):
            self.buffer[i].put(s,a,r,s_next,done)
